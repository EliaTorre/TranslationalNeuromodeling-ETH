...
function [accuracies_rf_laplacian_perm, accuracies_adb_laplacian_perm, accuracies_knn_laplacian_perm, accuracies_svm_laplacian_perm] = accuracies_LAPLACE_perm(data_drugs, permutations, seed)
    ...
    opts = {'Distance', 'Euclidean', 'Sigma', 1, 'NumNeighbors', 2, 'NumDimensions', 2, 'Verbose', 1};
    
    accuracies_rf_laplacian_perm = zeros(permutations,7);
    accuracies_adb_laplacian_perm = zeros(permutations,7);
    accuracies_knn_laplacian_perm = zeros(permutations,7);
    accuracies_svm_laplacian_perm = zeros(permutations,7);
    ...
    for perm = 1:permutations
        for drug = 1:7
            ...
            cp_rf_laplacian_perm = classperf(y);
            cp_knn_laplacian_perm = classperf(y);
            cp_svm_laplacian_perm = classperf(y);
            cp_adb_laplacian_perm = classperf(y);
            ...
            for i = 1:10
                ...
                class_rf_laplacian_perm = predict(rf, X(test,:));
                
                % AdaBoost
                if drug == 7
                    % AdaBoostM2 for Multi-Class
                    adb = fitcensemble(X(train,:), y(train,:), 'Method', 'AdaBoostM2');
                    class_adb_laplacian_perm = predict(adb, X(test,:));
                else
                    % AdaBoostM1 for Binary-Class
                    adb = fitcensemble(X(train,:), y(train,:), 'Method', 'AdaBoostM1');
                    class_adb_laplacian_perm = predict(adb, X(test,:));
                end
        
                % KNN
                knn = fitcknn(X(train,:), y(train,:), 'NumNeighbors', 9, 'Distance', 'cosine');
                class_knn_laplacian_perm = predict(knn, X(test,:));
        
                % SVM
                svm = fitcecoc(X(train,:), y(train,:));
                class_svm_laplacian_perm = predict(svm, X(test,:));
                
                % Update the class performance objects
                classperf(cp_rf_laplacian_perm, class_rf_laplacian_perm, test);
                classperf(cp_knn_laplacian_perm, class_knn_laplacian_perm, test);
                classperf(cp_svm_laplacian_perm, class_svm_laplacian_perm, test);
                classperf(cp_adb_laplacian_perm, class_adb_laplacian_perm, test);
            end
        
            accuracy_rf_laplacian_perm = 1 - cp_rf_laplacian_perm.ErrorRate;
            accuracy_knn_laplacian_perm = 1 - cp_knn_laplacian_perm.ErrorRate;
            accuracy_svm_laplacian_perm = 1 - cp_svm_laplacian_perm.ErrorRate;
            accuracy_adb_laplacian_perm = 1 - cp_adb_laplacian_perm.ErrorRate;
            accuracies_rf_laplacian_perm(perm, drug) = accuracy_rf_laplacian_perm;
            accuracies_knn_laplacian_perm(perm, drug) = accuracy_knn_laplacian_perm;
            accuracies_svm_laplacian_perm(perm, drug) = accuracy_svm_laplacian_perm;
            accuracies_adb_laplacian_perm(perm, drug) = accuracy_adb_laplacian_perm;
        end
    end
end