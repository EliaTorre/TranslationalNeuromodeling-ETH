% Input to the function: data_drugs table and a Random Seed value.
% Output: 4 lists of accuracies for: RandomForest Tuned, AdaBoost Tuned,
% KNN Tuned, Support Vector Machine Tuned. 
% Each list contains Binary and Multi-Class Accuracies (in the following order):
% Placebo vs Galantamine, Placebo vs Amisulpride, Placebo vs Levodopa,
% Placebo vs Biperdine, Biperdine vs Galantamine, Amisulpride vs Levodopa,
% Placebo and 4 Drugs (5 Class) Classification.

function [accuracies_rf_laplacian, accuracies_adb_laplacian, accuracies_knn_laplacian, accuracies_svm_laplacian] = accuracies_LAPLACE(data_drugs, seed)
    X = table2array(data_drugs(:,3:end));
    y = table2array(data_drugs(:,2));
    y_cat = categorical(y);
    
    placebo_idx = find(y_cat == 'Placebo');
    amisulpride_idx = find(y_cat == 'Amisulpride');
    biperdine_idx = find(y_cat == 'Biperdine');
    galantamine_idx = find(y_cat == 'Galantamine');
    levodopa_idx = find(y_cat == 'Levodopa');
    
    X_galantamine_placebo = vertcat(X(placebo_idx,:), X(galantamine_idx,:));
    y_galantamine_placebo = vertcat(y(placebo_idx,:), y(galantamine_idx,:));
    
    X_amisulpride_placebo = vertcat(X(placebo_idx,:), X(amisulpride_idx,:));
    y_amisulpride_placebo = vertcat(y(placebo_idx,:), y(amisulpride_idx,:));
    
    X_levodopa_placebo = vertcat(X(placebo_idx,:), X(levodopa_idx,:));
    y_levodopa_placebo = vertcat(y(placebo_idx,:), y(levodopa_idx,:));
    
    X_biperdine_placebo = vertcat(X(placebo_idx,:), X(biperdine_idx,:));
    y_biperdine_placebo = vertcat(y(placebo_idx,:), y(biperdine_idx,:));
    
    X_biperdine_galantamine = vertcat(X(galantamine_idx,:), X(biperdine_idx,:));
    y_biperdine_galantamine = vertcat(y(galantamine_idx,:), y(biperdine_idx,:));
    
    X_amisulpride_levodopa = vertcat(X(amisulpride_idx,:), X(levodopa_idx,:));
    y_amisulpride_levodopa = vertcat(y(amisulpride_idx,:), y(levodopa_idx,:));
    
    Xs = {X_galantamine_placebo, X_amisulpride_placebo, X_levodopa_placebo, X_biperdine_placebo, X_biperdine_galantamine, X_amisulpride_levodopa, X};
    ys = {y_galantamine_placebo, y_amisulpride_placebo, y_levodopa_placebo, y_biperdine_placebo, y_biperdine_galantamine, y_amisulpride_levodopa, y};

    rng(seed);

    % Set options for embedding
    opts = {'Distance', 'Euclidean', 'Sigma', 1, 'NumNeighbors', 10, 'NumDimensions', 2, 'Verbose', 1};
    
    % Define the number of trees in the ensemble

    num_trees = 50;
    min_leaf_size = 18;
    max_num_splits = 18;
    num_PTS = 1;
    
    % Define Hyperparameters.
    
    t_svm = templateSVM('BoxConstraint', 0.045284, 'KernelFunction', 'linear', 'KernelScale', 0.0019785);

    t_adb = templateTree('MinLeafSize', 71, 'MaxNumSplits', 3);
    
    accuracies_rf_laplacian = zeros(1,7);
    accuracies_adb_laplacian = zeros(1,7);
    accuracies_knn_laplacian = zeros(1,7);
    accuracies_svm_laplacian = zeros(1,7);
    
    for drug = 1:7
        X = Xs{drug};
        X = laplacianEigenmaps(X, opts{:});
        y = ys{drug};
        
        cv = cvpartition(y,'Kfold', 10, 'Stratify', true);
        
        % Initialize class performance objects
        cp_rf_laplacian = classperf(y);
        cp_knn_laplacian = classperf(y);
        cp_svm_laplacian = classperf(y);
        cp_adb_laplacian = classperf(y);
        
        % Perform 10-fold cross-validation
        for i = 1:10
            train = cv.training(i); 
            test = cv.test(i);
            
            % TreeBagger
            rf_laplacian = TreeBagger(num_trees, X(train,:), y(train,:), 'Method', 'classification', 'MinLeafSize', min_leaf_size, 'MaxNumSplits', max_num_splits, 'NumPredictorsToSample', num_PTS);
            class_rf_laplacian = predict(rf_laplacian, X(test,:));
            
            % AdaBoost
            if drug == 7
                % AdaBoostM2 for Multi-Class
                adb_laplacian = fitcensemble(X(train,:), y(train,:), 'Method', 'AdaBoostM2', 'NumLearningCycles', 13, 'Learners', t_adb, 'LearnRate', 0.008179);
                class_adb_laplacian = predict(adb_laplacian, X(test,:));
            else
                % AdaBoostM1 for Binary-Class
                adb_laplacian = fitcensemble(X(train,:), y(train,:), 'Method', 'AdaBoostM1', 'NumLearningCycles', 13, 'Learners', t_adb, 'LearnRate', 0.008179);
                class_adb_laplacian = predict(adb_laplacian, X(test,:));
            end
    
            % KNN
            knn_laplacian = fitcknn(X(train,:), y(train,:), 'NumNeighbors', 4, 'Distance', 'cosine');
            class_knn_laplacian = predict(knn_laplacian, X(test,:));
    
            % SVM
            svm_laplacian = fitcecoc(X(train,:), y(train,:), 'Coding', 'onevsone', 'Learners', t_svm);
            class_svm_laplacian = predict(svm_laplacian, X(test,:));
            
            % Update the class performance objects
            classperf(cp_rf_laplacian, class_rf_laplacian, test);
            classperf(cp_knn_laplacian, class_knn_laplacian, test);
            classperf(cp_svm_laplacian, class_svm_laplacian, test);
            classperf(cp_adb_laplacian, class_adb_laplacian, test);
        end
    
        accuracy_rf_laplacian = 1 - cp_rf_laplacian.ErrorRate;
        accuracy_knn_laplacian = 1 - cp_knn_laplacian.ErrorRate;
        accuracy_svm_laplacian = 1 - cp_svm_laplacian.ErrorRate;
        accuracy_adb_laplacian = 1 - cp_adb_laplacian.ErrorRate;
        accuracies_rf_laplacian(drug) = accuracy_rf_laplacian;
        accuracies_knn_laplacian(drug) = accuracy_knn_laplacian;
        accuracies_svm_laplacian(drug) = accuracy_svm_laplacian;
        accuracies_adb_laplacian(drug) = accuracy_adb_laplacian;
    end
end